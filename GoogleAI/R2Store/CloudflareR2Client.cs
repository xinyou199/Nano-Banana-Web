...
using CloudflareR2.NET.Factories;
using System.Net.Http.Headers;
using CloudflareR2.NET.Configuration;

namespace CloudflareR2.NET
{
    public class CloudflareR2Client : ICloudflareR2Client
    {
        ...
        public async Task<string> UploadBlobAsync(Stream blob, string blobName, CancellationToken cancellationToken, string contentType)
        {
            try
            {
                _logger.LogInformation("Starting upload for {BlobName}", blobName);
                string presignedUrl = await GeneratePresignedUrl(_bucketName, blobName, HttpVerb.PUT, _presignedUrlExpiry);
                string uploadUrl = presignedUrl.Split('?')[0];
                await UploadFileUsingPresignedUrl(presignedUrl, blob, contentType);
                _logger.LogInformation("Upload successful for {BlobName}", blobName);
                return uploadUrl;
            }
            catch (Exception ex)
        ...
        private static async Task UploadFileUsingPresignedUrl(string presignedUrl, Stream stream, string contentType = "application/zip")
        {
